...
class BlurredCamera(
    ...
    TextureShaderProgram(
        context,
        R.raw.camera_blur_vertex_shader,
        R.raw.camera_blur_fragment_shader
    ),
    CanvasRendererLayer {
    ...
    override fun onDrawFrame() {
        ...
        // log("viewport width = $viewPortWidth height = $viewPortHeight, byteBufferWidth = $byteBufferWidth byteBufferHeight = $byteBufferHeight")

        // log("resolution location = ${getResolutionLocation()} viewPortWidth = $viewPortWidth viewPortHeight = $viewPortHeight")
        GLES20.glUniform2f(
            //NOTE! these are flipped because this is what we set to
            // setDefaultBufferSize
            getResolutionLocation(),
            1920.toFloat(),
            1080.toFloat()
        )
    ...
    private fun startCamera() {
        ...
        cameraProviderFuture.addListener({
            ...
            preview.setSurfaceProvider { surfaceRequest ->
                ...
                log("surfaceRequest.resolution.width = ${surfaceRequest.resolution.width} surfaceRequest.resolution.height = ${surfaceRequest.resolution.height}")
    ...
    fun segmentMaskCallback(it: SegmentationMask) {
        coroutineScope.launch(Dispatchers.IO) {
            ...
            val buffer = ByteBuffer.allocateDirect(maskWidth * maskHeight * 4)
                .order(ByteOrder.nativeOrder()) // 4 bytes for RGBA
